...
class CollectionRouter():
    ...
    def setup(self):
        ...
        try:
            self.rl = SemanticRouter(encoder=self.cohere_encoder, routes=self.routes)
        except Exception as e:
            logger.warning(f"Cohere encoder failed, falling back to OpenAI: {e}")
            self.rl = SemanticRouter(encoder=self.openai_encoder, routes=self.routes)
